...
//
//  Device code for acceleration of the dpd/coul/slater/cut pair style
//
...
__kernel void k_dpd_coul_slater_cut(const __global numtyp4 *restrict x_,
  ...
  if (ii<inum) {
    ...
    for ( ; nbor<nbor_end; nbor+=n_stride) {
      ...
      if (rsq<cutsq[mtype].x) {
        ...
        if ( qtmp != 0.0 && extra[j].x != 0.0 && rsq < cutsq[mtype].z ){
          numtyp r2inv=ucl_recip(rsq);
          numtyp rinv = ucl_recip(r);
          numtyp rlamdainv = r * lamdainv;
          numtyp exprlmdainv = ucl_exp((numtyp)-2.0*rlamdainv);

          numtyp prefactor = qqrd2e * extra[j].x * qtmp * rinv;
          numtyp bracket_term = (numtyp)1 - exprlmdainv*( (numtyp)1 + ( (numtyp)2.0*rlamdainv*( (numtyp)1 + rlamdainv ) ) );
          force_coul = factor_coul * prefactor * bracket_term * r2inv;

          if (EVFLAG && eflag) {
            numtyp e_slater = (numtyp)1 - ((numtyp)1 + rlamdainv)*exprlmdainv;
            numtyp e = factor_coul * prefactor * e_slater;
            e_coul += e;
          }
        } // if cut_coulsq
...
__kernel void k_dpd_coul_slater_cut_fast(const __global numtyp4 *restrict x_,
                         const __global numtyp4 *restrict extra,
