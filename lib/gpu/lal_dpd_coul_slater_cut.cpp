/***************************************************************************
                                 lal_dpd_coul_slater_cut.cpp
                             -------------------
                            Eddy BARRAUD (IFPEN/Sorbonne)

  Class for acceleration of the dpd/coul/slater/cut pair style.
...
#include "dpd_coul_slater_cut_cl.h"
#elif defined(USE_CUDART)
const char *dpd_coul_slater_cut=0;
#else
#include "dpd_coul_slater_cut_cubin.h"
#endif

#include "lal_dpd_coul_slater_cut.h"
#include <cassert>
namespace LAMMPS_AL {
#define DPDCoulSlaterCutT DPDCoulSlaterCut<numtyp, acctyp>

extern Device<PRECISION,ACC_PRECISION> device;

template <class numtyp, class acctyp>
DPDCoulSlaterCutT::DPDCoulSlaterCut() : BaseDPD<numtyp,acctyp>(), _allocated(false) {
}

template <class numtyp, class acctyp>
DPDCoulSlaterCutT::~DPDCoulSlaterCut() {
  clear();
}

template <class numtyp, class acctyp>
int DPDCoulSlaterCutT::bytes_per_atom(const int max_nbors) const {
  return this->bytes_per_atom_atomic(max_nbors);
}

template <class numtyp, class acctyp>
int DPDCoulSlaterCutT::init(const int ntypes,
               ...
               const double qqrd2e, double lamda) {
  ...
  success=this->init_atomic(nlocal,nall,max_nbors,maxspecial,cell_size,
                            gpu_split,_screen,dpd_coul_slater_cut,"k_dpd_coul_slater_cut",onetype, extra_fields, need_charges);
...
template <class numtyp, class acctyp>
double DPDCoulSlaterCutT::host_memory_usage() const {
  return this->host_memory_usage_atomic()+sizeof(DPDCoulSlaterCut<numtyp,acctyp>);
}
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
int DPDCoulSlaterCutT::loop(const int eflag, const int vflag) {
...
template <class numtyp, class acctyp>
void DPDCoulSlaterCutT::update_coeff(int ntypes, double **host_a0, double **host_gamma,
...
template <class numtyp, class acctyp>
void DPDCoulSlaterCutT::get_extra_data(double *host_q) {
  q = host_q;
}

template class DPDCoulSlaterCut<PRECISION,ACC_PRECISION>;
}
