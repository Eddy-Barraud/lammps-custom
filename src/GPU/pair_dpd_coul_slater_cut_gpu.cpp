...
#include "pair_dpd_coul_slater_cut_gpu.h"
...
int dpd_coul_slater_cut_gpu_init(const int ntypes, double **cutsq, double **host_a0,
                                  ...
                                  const double lamda);
void dpd_coul_slater_cut_gpu_clear();
int **dpd_coul_slater_cut_gpu_compute_n(const int ago, const int inum_full, const int nall,
                                         ...
                                         double *prd);
void dpd_coul_slater_cut_gpu_compute(const int ago, const int inum_full, const int nall,
...
void dpd_coul_slater_cut_gpu_get_extra_data(double *host_q);

double dpd_coul_slater_cut_gpu_bytes();
...
PairDPDCoulSlaterCutGPU::PairDPDCoulSlaterCutGPU(LAMMPS *lmp) : PairDPDCoulSlaterCut(lmp), gpu_mode(GPU_FORCE)
{
  respa_enable = 0;
  reinitflag = 0;
  cpu_time = 0.0;
...
PairDPDCoulSlaterCutGPU::~PairDPDCoulSlaterCutGPU()
{
  dpd_coul_slater_cut_gpu_clear();
}

/* ---------------------------------------------------------------------- */

void PairDPDCoulSlaterCutGPU::compute(int eflag, int vflag)
{
  ev_init(eflag, vflag);
  ...
  double *q = atom->q;
  dpd_coul_slater_cut_gpu_get_extra_data(q);
  ...
  if (gpu_mode != GPU_FORCE) {
    ...
    inum = atom->nlocal;
    firstneigh = dpd_coul_slater_cut_gpu_compute_n(
    ...
    dpd_coul_slater_cut_gpu_compute(neighbor->ago, inum, nall, atom->x, atom->type, ilist, numneigh, firstneigh,
...
void PairDPDCoulSlaterCutGPU::init_style()
{

  if (comm->ghost_velocity == 0)
    error->all(FLERR,"Pair dpd requires ghost atoms store velocity");
  if (!atom->q_flag)
    error->all(FLERR,"Pair style coul/slater/cut requires atom attribute q");
  ...
  int success =
      dpd_coul_slater_cut_gpu_init(atom->ntypes + 1, cutsq, a0, gamma, sigma,
                   ...
                   force->special_coul, force->qqrd2e, lamda);
...
double PairDPDCoulSlaterCutGPU::memory_usage()
{
  double bytes = Pair::memory_usage();
  return bytes + dpd_coul_slater_cut_gpu_bytes();
}

/* ---------------------------------------------------------------------- */

void PairDPDCoulSlaterCutGPU::cpu_compute(int start, int inum, int eflag, int /* vflag */, int *ilist,
