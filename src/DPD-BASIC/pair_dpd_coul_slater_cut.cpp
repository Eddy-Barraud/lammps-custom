...
#include "pair_dpd_coul_slater_cut.h"
...
PairDPDCoulSlaterCut::PairDPDCoulSlaterCut(LAMMPS *lmp) :
...
PairDPDCoulSlaterCut::~PairDPDCoulSlaterCut()
{
  if (copymode) return;
...
void PairDPDCoulSlaterCut::compute(int eflag, int vflag)
{
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (qtmp != 0 && q[j] != 0 && rsq < cut_slatersq[itype][jtype]){
          ...
          forcecoul = factor_coul * qqrd2e * qtmp * q[j] * rinv * bracket_term * r2inv;
...
void PairDPDCoulSlaterCut::allocate()
{
  int i,j;
  allocated = 1;
  int n = atom->ntypes;
...
void PairDPDCoulSlaterCut::settings(int narg, char **arg)
{
  ...
  if (seed <= 0)
    error->all(FLERR,"Invalid random seed {} for pair_style dpd/coul/slater/cut command", seed);
  delete random;
...
void PairDPDCoulSlaterCut::coeff(int narg, char **arg)
{
  if (narg < 4 || narg > 6)
...
void PairDPDCoulSlaterCut::init_style()
{
...
double PairDPDCoulSlaterCut::init_one(int i, int j)
{
...
void PairDPDCoulSlaterCut::write_restart(FILE *fp)
{
...
void PairDPDCoulSlaterCut::read_restart(FILE *fp)
{
...
void PairDPDCoulSlaterCut::write_restart_settings(FILE *fp)
{
...
void PairDPDCoulSlaterCut::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairDPDCoulSlaterCut::write_data(FILE *fp)
{
...
void PairDPDCoulSlaterCut::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairDPDCoulSlaterCut::single(int i, int j, int itype, int jtype, double rsq,
                                     double factor_coul, double factor_dpd, double &fforce)
{
  double r,rinv,wd,phi;
  double forcecoul,phicoul;
  double r2inv, bracket_term;
  double energy = 0.0;
  fforce = 0.0;
  ...
  if (atom->q[i]*atom->q[j] != 0.0 && rsq < cut_slatersq[itype][jtype]) {
    ...
    bracket_term = 1 - exp(-2*r/lamda)*(1 + (2*r/lamda*(1+r/lamda)));
    forcecoul = factor_coul * force->qqrd2e * atom->q[i] * atom->q[j]  * rinv * bracket_term * r2inv;
    fforce += forcecoul;

    phicoul = factor_coul * force->qqrd2e * atom->q[i] * atom->q[j] * rinv *
            (1 - (1 + r/lamda)*exp(-2*r/lamda));
    energy += phicoul;
  }
...
void *PairDPDCoulSlaterCut::extract(const char *str, int &dim)
{
  dim = 0;
